...
import com.example.ex01.domain.WorkVO;
...
//    총점과 평균 점수 화면에 출력
    ...
    public RedirectView infoRegister(TaskVO taskVO, RedirectAttributes redirectAttributes){
        redirectAttributes.addFlashAttribute(taskVO);
        return new RedirectView("/ex/result");
    }
...
//    - user.html : 일반 회원 페이지 출력
    ...
    public RedirectView checkId(MemberVO memberVO , RedirectAttributes redirectAttributes){
        /*redirectAttributes.addAttribute("identification", memberVO.getIdentification());
        redirectAttributes.addAttribute("password", memberVO.getPassword());*/


        if(memberVO.getIdentification().equals("admin")){
            return new RedirectView("/ex/admin");
        } else if (memberVO.getIdentification().equals("user")){
            return new RedirectView(("/ex/user"));
        } else{
            return new RedirectView("/ex/login");
        }

    }

    @GetMapping("admin")
    public String admin(MemberVO memberVO){
        return "Login/admin";
    }

    @GetMapping("user")
    public String user(MemberVO memberVO){
        return "Login/user";
    }
...
//    - work.html
    ...
    public String getToWork(String name) {
        ...
        boolean lateCondition = LocalTime.now().compareTo(LocalTime.of(9, 0,0)) == 1;
    ...
    public String leaveWork(String name){
        ...
        /*boolean leaveWorkCondition = hour >= 9 && minute > 0;*/
        boolean lateCondition = LocalTime.now().compareTo(LocalTime.of(0, 0,0)) == 1;

        return "/check-in/" + (lateCondition ? "late" : "work");
    }
